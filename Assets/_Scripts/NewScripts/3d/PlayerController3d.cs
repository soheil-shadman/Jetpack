...
public class PlayerController3d : MonoBehaviour
{
    ...
    [SerializeField] private float _bgHorizentalOffset = 1.5f;
    ...
    void Start()
    {
        ...
        _rigidbody2D = this.GetComponent<Rigidbody>();
    ...
    private void FixedUpdate()
    {
        ...
        _rigidbody2D.AddForce(Physics.gravity * _gravityScale, ForceMode.Acceleration);
    
    }
    // void FixedUpdate () {
    //     _rigidbody2D.AddForce(Vector3.down * _gravityScale * _rigidbody2D.mass);
    // }
    ...
    private void Update()
    {
        ...
        Collider[]  hitObject = Physics.OverlapSphere(groundCheck.position, groundCheckRadius,layerMask:groundLayerMask);
        _isGrounded = hitObject.Length!=0 && (_rigidbody2D.velocity.y-3.5f) <= 0;
        _outOfFuel = _currentFuel <= 0 ;
        ...
        if (transform.position.x >= (_backGroundwidth / 2 - _bgHorizentalOffset))
        {
            var vec3 = transform.position;
            vec3.x = vec3.x - _backGroundwidth+_bgHorizentalOffset;
            transform.position = vec3;

        }
        if (transform.position.x <= -1*(_backGroundwidth / 2 - _bgHorizentalOffset))
        {
